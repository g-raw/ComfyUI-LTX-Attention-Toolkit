...
- **Universal attention hook** (`core/hooks.py:_make_full_hook`): Installed on both `optimized_attention` and `optimized_attention_masked. Priority order per call: (1) Profiling → AttentionStore, (2) QKV Capture → QKVStore, (3) QKV Transfer substitution, (4) QKV Multiplier per-head Q/K/V/O scaling, (5) Head Freeze map injection, (6) Normal pass-through.
...
- **LTX Attn — QKV Multiplier** (`nodes/transfer.py:LTXQKVMultiplier`, `ops/qkv_multiply.py:apply_qkv_multiply`): Live per-head Q/K/V/output scaling, no prior capture needed. `targets` uses the same `parse_block_head_pairs` format as Head Freeze/QKV Transfer, plus a whole-string `"all"`/`"all:all"` for every block (0-47) and head (0-31) — resolved directly (no store to check against, unlike QKV Transfer's `"all"`). `q_mult`/`k_mult` scale Q/K before the attention dot product (softmax logit scale — changes attention sharpness, does **not** ablate: 0 just makes the head attend uniformly, it still contributes via V). `v_mult`/`o_mult` scale V and the head's slice of the pre-output-projection attention output respectively (true magnitude control — 0 genuinely zeroes the head's contribution; `o_mult` is mathematically equivalent to scaling that head's columns of the block's shared output projection). All four multipliers are shared across every target, same as Head Freeze's `blend_weight`. `apply_sa`/`apply_ca` gate which attention type(s) it fires on. `from_step`/`to_step` (default full range `0`-`999`) gate which denoising steps it's active for, per targeted block's own step counter — same per-block `step_counters` pattern as QKV Transfer. Same blank-`targets`-disables convention as Head Freeze/QKV Transfer (clone+unwrap+passthrough, reliable regardless of ComfyUI bypass/mute). Dispatches via `core/hooks.py`'s `_qkvmul_active`/`_qkvmul_cfg` transformer_options keys, priority 4 (after QKV Transfer, before Head Freeze).
